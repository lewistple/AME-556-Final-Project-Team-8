...
Q = diag([500 500 100 250 250 150 1]); 
...
function J = ComputeJacobianSymbolic()
    ...
    % 足端1的位置 (身体坐标系)
    x1 = x + leg_l*sin(q1 + q2 + theta) + leg_l*sin(q1 + theta) + sin(theta)*body_a/2;
    y1 = y - leg_l*cos(q1 + q2 + theta) - leg_l*cos(q1 + theta) - cos(theta)*body_a/2;

    % 足端2的位置 (身体坐标系)
    x2 = x + leg_l*sin(q3 + q4 + theta) + leg_l*sin(q3 + theta) + sin(theta)*body_a/2;
    y2 = y - leg_l*cos(q3 + q4 + theta) - leg_l*cos(q3 + theta) - cos(theta)*body_a/2;
